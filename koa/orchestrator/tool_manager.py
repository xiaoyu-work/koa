...
class ToolManagerMixin:
    ...
    async def _build_tool_schemas(
        ...
        agent_tool_names = [
            s.get("function", {}).get("name", s.get("name", "?")) for s in agent_tool_schemas
        ]
        logger.info(
    ...
    def _build_builtin_tools(self) -> List[AgentTool]:
        ...
        from ..builtin_agents.location import (
            ...
            get_user_location_executor,
            set_location_reminder_executor,
        )
        from ..builtin_agents.tools.download_image import (
            DOWNLOAD_IMAGE_SCHEMA,
            download_image_executor,
        )
        ...
        from ..builtin_agents.tools.action_history import (
            ...
            recall_recent_actions_executor,
        )
    ...
    def _build_tool_metadata(self, metadata: Optional[dict] = None) -> dict:
        ...
        if self.database:
            from koa.builtin_agents.digest.important_dates_repo import ImportantDatesRepository
    ...
    async def _execute_single(
        ...
        if self._is_agent_tool(tool_name):
            ...
            tools = (
                request_tools if request_tools is not None else getattr(self, "builtin_tools", [])
            )
            ...
            if pipeline is not None:
                ...
                if not pipeline_result.success and isinstance(
                    pipeline_result.result, BaseException
                ):
    ...
    def _cap_tool_result(self, result_text: str) -> str:
        ...
        if newline_pos > 0:
            cut = newline_pos
        logger.warning(f"[ReAct] Tool result truncated: {len(result_text)} -> {cut} chars")

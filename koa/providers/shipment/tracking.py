...
from .carrier_detector import get_tracking_url, normalize_tracking_number
...
class TrackingProvider:
    ...
    async def _register(self, tracking_number: str, carrier_code: int = None) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if accepted:
                item = accepted[0]
                logger.info(
                    f"Successfully registered {tracking_number} with carrier {item.get('carrier')}"
                )
                return {
                    "success": True,
                    "tracking_number": tracking_number,
    ...
    async def _get_track_info(
        self, tracking_number: str, carrier_code: int = None
    ) -> Dict[str, Any]:
    ...
    def _get_user_friendly_error(self, error_type: str, details: Any = None) -> str:
        ...
        if error_type == "connection_error":
            ...
            if "invalid" in error_msg or "format" in error_msg:
                ...
                return (
                    "Couldn't add this tracking number. Please verify it's correct and try again."
                )
